    ...
    def apply_super_power(self, boss, heroes):
        for hero in heroes:
            if hero.health <= 0:
                ...
                self.health = 0
                print(f'{self.name} вазродиль {hero.name} пожертвовав собой')
    ...
    def apply_super_power(self, boss, heroes):
        ...
        if round_number >= 1:
            for hero in heroes:
                if hero.health > 0:
                    hero.damage += 10
            print(f'{self.name} забустил товарищей')
...
class Hacker(Hero):
    ...
    def apply_super_power(self, boss, heroes):
        ...
        if round_number % 2 == 0:
            ...
            hero = choice(heroes)
            hero.health += 50
            print(f'{self.name} украл у {boss.name} 50хп и добавил {hero.name}')
...
def start_game():
    ...
    witcher = Witcher('Max', 200, 0)
    hacker = Hacker('Alex', 150, 5)


    heroes_list = [warrior_1, warrior_2, magic, berserk, doc, assistant, witcher, hacker]
